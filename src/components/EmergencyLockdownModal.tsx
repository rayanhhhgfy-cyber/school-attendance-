    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-xs">
        <motion.div
          ...
          className="bg-white dark:bg-neutral-900 rounded-2xl max-w-lg w-full p-5 sm:p-6 shadow-2xl border-2 border-red-600 text-slate-900 dark:text-white"
          role="dialog"
          aria-modal="true"
        >
          {/* Top Header */}
          <div className="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-neutral-800">
            <div className="flex items-center gap-2.5">
              <div className="w-10 h-10 rounded-xl bg-red-100 dark:bg-red-950 text-red-700 dark:text-red-400 flex items-center justify-center flex-shrink-0">
          ...
          <div className="py-4 space-y-3">
            {settings.emergencyLockdown ? (
              <div className="p-3.5 bg-red-50 dark:bg-red-950/40 border border-red-400 dark:border-red-900/60 rounded-xl text-red-900 dark:text-red-200">
                <div className="flex items-center gap-2 font-bold text-sm sm:text-base mb-1">
                  <Lock className="w-4 h-4 text-red-700 dark:text-red-400" />
                ...
                {settings.lockdownTime && (
                  <p className="text-xs font-semibold mt-1.5 text-red-700 dark:text-red-400">
                    توقيت التفعيل: {settings.lockdownTime}
                  </p>
                )}
              </div>
            ) : (
              <div className="p-3.5 bg-amber-50 dark:bg-amber-950/40 border border-amber-300 dark:border-amber-900/60 rounded-xl text-amber-950 dark:text-amber-200">
                <div className="flex items-center gap-2 font-bold text-sm sm:text-base mb-1">
                  <AlertTriangle className="w-4 h-4 text-amber-700 dark:text-amber-400" />
                  <span>ماذا يحدث عند تفعيل الإغلاق الطارئ؟</span>
                </div>
                <ul className="text-xs sm:text-sm space-y-1 list-disc list-inside font-medium text-amber-900 dark:text-amber-300">
            ...
            {!settings.emergencyLockdown && (
              <div>
                ...
                <select
                  ...
                  className="w-full h-10 px-3 bg-slate-100 dark:bg-neutral-800 border border-slate-300 dark:border-neutral-700 rounded-xl text-slate-900 dark:text-white font-medium text-xs sm:text-sm focus:border-red-600 focus:outline-hidden cursor-pointer"
                >
